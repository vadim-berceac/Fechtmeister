...
public class TargetRigController : ManagedUpdatableObject, ITargetRigWeightController
{
    ...
    private float _desiredWeight = 0f; // Новое поле для ручного контроля
    ...
    {
        // Приоритет: ручное управление через интерфейс, затем CharacterCore
        if (_desiredWeight > 0f)
        {
        ...
        return CharacterCore.CurrentState.ItemTargetingRigWeight;
    }

    // Реализация интерфейса ITargetRigWeightController
    public void SetDesiredWeight(float weight)
    {
        _desiredWeight = Mathf.Clamp01(weight);
    }
    ...
    public void SetLookAtEnabled(bool enabled)
    {
        ...
        _desiredWeight = _currentTargetWeight;
    }
